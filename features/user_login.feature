Feature: User(user)Login - Sign in to your account
  ...
  Scenario: Successful login with valid credentials
    When the user enters email "user@example.com" and password "user123"
  ...
  Scenario: Login fails with wrong password
    When the user enters email "user1@example.com" and password "user1234"
    And the user clicks the "Sign In" button
  ...
  Scenario: Login fails with invalid email format
    When the user enters email "notanemail" and password "user123"
    And the user clicks the "Sign In" button
